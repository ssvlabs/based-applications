...
contract SSVBasedApps is ISSVBasedApps, UUPSUpgradeable, Ownable2StepUpgradeable, IPlatformManager, IStrategyManager {
    ...
    function initialize(
        address owner_,
        IPlatformManager ssvPlatformManager_,
    ...
    function __SSVBasedApplications_init_unchained(
        ...
        s.ssvContracts[SSVBasedAppsModules.SSV_PLATFORM_MANAGER] = address(ssvPlatformManager_);
    ...
    function updateBAppMetadataURI(string calldata metadataURI) external {
        _delegateTo(SSVBasedAppsModules.SSV_PLATFORM_MANAGER);
    }
    ...
    function createObligation(uint32 strategyId, address bApp, address token, uint32 obligationPercentage) external {
        _delegateTo(SSVBasedAppsModules.SSV_STRATEGY_MANAGER);
    }
    ...
    function updateModules(SSVBasedAppsModules[] calldata moduleIds, address[] calldata moduleAddresses)
        external
        onlyOwner
    {
        uint32 size;
        for (uint256 i; i < moduleIds.length; i++) {
            ...
            SSVBasedAppsStorage.load().ssvContracts[moduleIds[i]] = moduleAddresses[i];

            emit ModuleUpdated(moduleIds[i], moduleAddresses[i]);
        }
    }

    function _delegateTo(SSVBasedAppsModules moduleId) internal {
        address implementation = SSVBasedAppsStorage.load().ssvContracts[moduleId];
        assembly {
